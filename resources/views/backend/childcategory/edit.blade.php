...
<div class="col-12">
    <div class="card">
        <div class="card-body">
            ...
            <form method="post" action="{{ route('childcategories.update') }}" id="myForm"  enctype="multipart/form-data" >
                @csrf
            ...
            <div class="row mb-3">
                ...
                <div class="form-group col-sm-5">
                    <select class="form-control" id="subcategory-dropdown"  required>
                        <option value="">Select category</option>
                        @if($categories)
                            @foreach($categories as $category)
                                <option value="{{$category->id}}" {{ $category->id==$maincategory?'selected':'' }} >{{$category->name}}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
                    ...
                    <select class="form-control" id="subcategory-dropdown" name="category" required>
                        ...
                        @if($subcategories)
                            @foreach($subcategories as $subcategory)
                                <option value="{{$subcategory->id}}" {{ $subcategory->id==$edit->parent_id?'selected':'' }} >{{$subcategory->name}}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
            ...
            <div class="row mb-3">
                ...
                <div class="form-group col-sm-5">
                    <input name="url_slug" class="form-control" type="text"  id="url_slug" value="{{ $edit->url_slug }}" >
                </div>
            </div>
