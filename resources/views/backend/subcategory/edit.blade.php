...
<div class="col-12">
    <div class="card">
        <div class="card-body">
            ...
            <form method="post" action="{{ route('subcategories.update') }}" id="myForm"  enctype="multipart/form-data" >
                @csrf
                    ...
                    <select class="form-control" name="category" required>
                        ...
                        @if($categories)
                            @foreach($categories as $category)
                                <option value="{{$category->id}}" {{ $category->id==$edit->parent_id?'selected':'' }} >{{$category->name}}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
