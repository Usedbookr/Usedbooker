...
{
    public function All(){
        $categories = Category::Where('level',3)->latest()->get();
        return view('backend.childcategory.all',compact('categories'));
    } // End Method 


    
    Public function ApiAll(){
        $categories = Category::Where('level',3)->Where('status',1)->get();
    ...
    Public function ApiChildcategory(Request $request, $id){
        $categories = Category::Where('level',3)->Where('parent_id',$id)->Where('status',1)->get();
        if($categories->isNotEmpty()){
            foreach($categories as $catlists){
                $catlists->images = asset($catlists->images);
            }
    ...
    public function Add(){
        ...
        return view('backend.childcategory.add',compact('categories'));
    } // End Method 
    ...
    public function Store(Request $request){
        ...
        Category::insert([
         ...
         $notification = array(
            'message' => 'Child-Category Inserted Successfully', 
            'alert-type' => 'success'
        );

        return redirect()->route('childcategories.all')->with($notification);
    ...
    public function Edit($id){
        ...
        // dd($edit->parent_id);
        $subcat = Category::Where('id',$edit->parent_id)->first();
        // dd($subcat);
        $maincategory = $subcat->parent_id ?? '';
        $categories = Category::Where('level',1)->Where('status',1)->get();
        $subcategories = Category::Where('level',2)->Where('status',1)->Where('parent_id',$maincategory)->get();
        // dd($subcategories);
        return view('backend.childcategory.edit',compact('edit','categories','subcategories','maincategory'));
